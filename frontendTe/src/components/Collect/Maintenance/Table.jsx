...
export function TableSortMaintenance() {
    ...
    const showTable = filteredRows.length > 0 ? (
        filteredRows.map((row) => (
            <tr key={row._id}>
                ...
                <td>
                    {row.startTime === "" ? (
                        <button type="button" className="btn btn-success" onClick={() => setidTicket(row._id)} data-bs-toggle="modal" data-bs-target="#CollectMaintenance">Collect</button>
                    ) : (
                        <>{row.startTime}</>
                    )}
                </td>
                ...
                <td>
                    {
                        row.startTime !== "" && (
                            <div className='d-flex gap-3 align-items-center'>
                                <i className="fa-regular fa-pen-to-square text-success fs-4 icon-cursor" onClick={() => setEditIdTicket(row._id)} data-bs-toggle="modal" data-bs-target="#EditMaintenance"></i>
                            </div>
                        )
                    }
                </td>
                <td>
                    {row.requiredSpareParts == "Norequired" ? (
                        <button type="button" onClick={() => setidTicket(row._id)} className="btn btn-danger" data-bs-toggle="modal" data-bs-target="#CloseTicketMaintenance">
                            Close
                        </button>
                    ) : row.approvedIT !== "" && row.pictureBefore.url !== "" && row.pictureAfter.url !== "" && row.report !== "" && (
                        <button type="button" onClick={() => setidTicket(row._id)} className="btn btn-danger" data-bs-toggle="modal" data-bs-target="#CloseTicketMaintenance">
                            Close
                        </button>
                    )}
